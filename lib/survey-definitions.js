...
import { SIMPLE_SURVEY_KEY } from "./constants";
...
function translateSimple(locale) {
  const normalizedLocale = normalizeLocale(locale);
  const translations = getTranslations(normalizedLocale);
  const simple = translations.assessment.simple;
  const isEnglish = normalizedLocale === "en";

  return {
    name: SIMPLE_SURVEY_KEY,
    title: isEnglish ? "PAKEM DIRI Simplified Version" : "PAKEM DIRI Versi Sederhana",
    pages: [
      {
        name: SIMPLE_SURVEY_KEY,
        title: `**${isEnglish ? "Security Assessment" : "Pemeriksaan Keamanan"} <span class='aksen'>${isEnglish ? "Simplified" : "Sederhana"}</span>**`,
        description: simple.description,
        elements: simple.items.map((title, index) => ({
          type: "radiogroup",
          name: `${SIMPLE_SURVEY_KEY}_${index + 1}`,
          title,
          correctAnswer: "1",
          isRequired: true,
          choices: [
    ...
    pageNextText: translations.survey.next,
    pagePrevText: translations.survey.previous,
    completeText: simple.submit,
    showPrevButton: true,
    firstPageIsStarted: false,
    startSurveyText: translations.survey.start,
    completedHtml: simple.submitted,
...
export function getSurveyDefinition(locale, categoryKey) {
  if (categoryKey === SIMPLE_SURVEY_KEY) return translateSimple(locale);
